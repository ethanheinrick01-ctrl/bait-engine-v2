...
def rank_candidates(candidates: list[CandidateReply]) -> list[CandidateReply]:
    ...
    for candidate in candidates:
        score = (
            0.34 * candidate.estimated_bite_score
            + 0.28 * candidate.estimated_audience_score
            + 0.22 * (1.0 - candidate.critic_penalty)
            + 0.16 * (1.0 if len(candidate.text.split()) <= 16 else 0.6)
        )
